...
Void BatchState_obj::activate( ::phoenix::Batcher batcher){
{
		...
		if ((tmp3)){
			...
			if ((tmp7)){
				...
				if ((tmp11)){
					HX_STACK_LINE(104)
					::phoenix::Rectangle tmp12 = batcher->view->viewport;		HX_STACK_VAR(tmp12,"tmp12");
					HX_STACK_LINE(104)
					...
					HX_STACK_LINE(106)
					{
						...
						HX_STACK_LINE(106)
						glScissor(tmp31,tmp32,tmp33,tmp34);
					}
				}
			}
		}
...
Void BatchState_obj::str( ){
{
		...
		if ((tmp15)){
			...
			HX_STACK_LINE(163)
			Dynamic tmp18 = hx::SourceInfo(HX_HCSTRING("BatchState.hx","\x07","\x6b","\xe4","\x91"),163,HX_HCSTRING("phoenix.BatchState","\x96","\x62","\xb8","\xcc"),HX_HCSTRING("str","\xb1","\xa8","\x57","\x00"));		HX_STACK_VAR(tmp18,"tmp18");
			HX_STACK_LINE(163)
		...
		if ((tmp45)){
			...
			HX_STACK_LINE(174)
			Dynamic tmp48 = hx::SourceInfo(HX_HCSTRING("BatchState.hx","\x07","\x6b","\xe4","\x91"),174,HX_HCSTRING("phoenix.BatchState","\x96","\x62","\xb8","\xcc"),HX_HCSTRING("str","\xb1","\xa8","\x57","\x00"));		HX_STACK_VAR(tmp48,"tmp48");
			HX_STACK_LINE(174)
